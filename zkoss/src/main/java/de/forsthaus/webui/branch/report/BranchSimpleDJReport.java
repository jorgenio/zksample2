...
public class BranchSimpleDJReport extends Window implements Serializable {
	...
	public void doPrint() throws JRException, ColumnBuilderException, ClassNotFoundException, IOException {
		...
		final String description = Labels.getLabel("common.Description");
		...
		final BrancheService as = (BrancheService) SpringUtil.getBean("brancheService");
		final List<Branche> resultList = as.getAlleBranche();

		// Create Datasource and put it in Dynamic Jasper Format
		final List data = new ArrayList(resultList.size());

		for (final Branche obj : resultList) {
